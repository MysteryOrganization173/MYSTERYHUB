# Mystery Bundle Hub — Architecture Reference

> Production SaaS platform for curated mystery bundles.
...
│   │   ├── home/               # Homepage sections (Hero, FeaturedBundles…)
...
│   │   └── bundles.ts          # bundlesService (getAll, getBySlug, getFeatured…)
│   │
│   ├── config/
...
│   │
│   ├── types/
│   │   └── index.ts            # Domain types: User, Bundle, Order, Review…
│   │
│   ├── utils/
...
`src/services/bundles.ts` — `bundlesService`:
- `getAll(params)` — paginated list with filters
- `getBySlug(slug)` — single bundle detail
- `getFeatured()` — homepage featured items
- `getRelated(id, limit)` — related bundles
...
All domain types live in `src/types/index.ts`. Core types:

- `User`, `UserRole`
- `Bundle`, `BundleCategory`, `BundleTier`, `BundleStatus`
- `Order`, `OrderStatus`, `Address`
- `Review`, `Referral`, `ReferralStatus`
- `ApiResponse<T>`, `PaginatedResponse<T>`
- `BundleFilters`, `SortOption`
- `LoadingState`
